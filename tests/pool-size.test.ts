...
describe('poolSize', () => {
  it('uses a single worker on a low-memory device', () => {
    ...
    expect(poolSize({ hardwareConcurrency: 8, deviceMemory: 1 })).toBe(1);
  });

  it('is bounded by memory headroom, not just cores', () => {
    expect(poolSize({ hardwareConcurrency: 8, deviceMemory: 4 })).toBe(2);
    expect(poolSize({ hardwareConcurrency: 2, deviceMemory: 8 })).toBe(2);
  });

  it('caps at POOL_MAX even with abundant resources', () => {
    expect(poolSize({ hardwareConcurrency: 16, deviceMemory: 16 })).toBe(POOL_MAX);
  });
